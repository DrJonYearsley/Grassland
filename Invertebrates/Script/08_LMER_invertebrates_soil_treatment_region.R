...
# dir.create(paste0(data.dir,"LMER model invertebrates soil trt region"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt region"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt region/Model validation"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt region/Model output"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt region/Residuals"))

mydir.data <- paste0(data.dir,"LMER model invertebrates soil trt region")
mydir <- paste0(figures.dir,"LMER model invertebrates soil trt region")
...
obs_var_trt <- c()
obs_var_region <- c()
obs_var_trt_region <- c()
obs_var_farm <- c()
resid_norm <- c()
resid_var_trt <- c()
resid_var_region <- c()
resid_var_trt_region <- c()
resid_var_farm <- c()
mod_AIC <- c()
mod_Rsq_marginal <- c()
...
for (daytime.hr in timepoint) {
  ...
  for (duration.hr in width) {
    ...
    df_joined <- full_join(df_inverts[, c("Region", "Farm", "Plot", 
                                          "site_ID", "day", 
                                          "treatment", "inverts_BC_all", "inverts_BC_soil")], 
    ...
    # a function Day since end of drought (numeric), the effect size of soil moisture (numeric),
    # treatment (Control vs. Drought) and Region (Border, Cork, Dublin and Limerick)
    # Random factors included: Farm 
    
    lmer_full_mod <- lmer(inverts_BC_all ~ day*soil_ES*treatment*Region + (1|Farm),
                          data = df_joined)
    ...
    farm_out.1 <- data.frame(rownames(coeff_random.1))
    random_out.1 <- data.frame("Farm" = farm_out.1$rownames.coeff_random.1., "Intercept" = coeff_random.1$`(Intercept)`)
    ...
    OBS_var_trt_region <- leveneTest(inverts_BC_all ~ treatment*Region, data = df_mod)
    
    if(OBS_var_trt_region$`Pr(>F)`[1] < 0.001){
      main.trt.region.0 <- "p < 0.001"
    }
    
    if(OBS_var_trt_region$`Pr(>F)`[1] >= 0.001 & OBS_var_trt_region$`Pr(>F)`[1] < 0.01){
      main.trt.region.0 <- "p < 0.01"
    }
    
    if(OBS_var_trt_region$`Pr(>F)`[1] >= 0.01 & OBS_var_trt_region$`Pr(>F)`[1] < 0.05){
      main.trt.region.0 <- "p < 0.05"
    }
    
    if(OBS_var_trt_region$`Pr(>F)`[1] >= 0.05 ){
      main.trt.region.0 <- parse(text = paste0('p == ', round(OBS_var_trt_region$`Pr(>F)`[1], digits = 3)))
    }
    
    box.obs.trt.region <- ggplot(df_mod, aes(x = Region, y = inverts_BC_all, fill = Region)) +
      geom_boxplot(outlier.shape = 21, alpha = 0.7) +
      ...
      labs(x = "Region",
           y = "Bray-Curtis Dissimilarity Index\nInvertebrates",
           title = "Invertebrates Bray-Crutis Dissimilarity Index ~ treatment*Region",
           subtitle = main.trt.region.0)
    ...
    obs.fit.trt.region <- ggplot(df_mod, aes(x = Fitted, y = inverts_BC_all, 
    ...
    RESID_var_trt_region <- leveneTest(Residuals ~ treatment*Region, data = df_mod)
    
    if(RESID_var_trt_region$`Pr(>F)`[1] < 0.001){
      main.trt.region <- "p < 0.001"
    }
    
    if(RESID_var_trt_region$`Pr(>F)`[1] >= 0.001 & RESID_var_trt_region$`Pr(>F)`[1] < 0.01){
      main.trt.region <- "p < 0.01"
    }
    
    if(RESID_var_trt_region$`Pr(>F)`[1] >= 0.01 & RESID_var_trt_region$`Pr(>F)`[1] < 0.05){
      main.trt.region <- "p < 0.05"
    }
    
    if(RESID_var_trt_region$`Pr(>F)`[1] >= 0.05 ){
      main.trt.region <- parse(text = paste0('p == ', round(RESID_var_trt_region$`Pr(>F)`[1], digits = 3)))
    }
    
    box.trt.region <- ggplot(df_mod, aes(x = Region, y = Residuals, fill = Region)) +
      geom_boxplot(outlier.shape = 21, alpha = 0.7) +
      facet_wrap(~ treatment) +
      theme_minimal() +
      theme(legend.position = "none",
      ...
      labs(x = "Region",
           y = "Residuals",
           title = "Residuals ~ treatment * Region",
           subtitle = main.trt.region)
    ...
    final.plot0 <- grid.arrange(plot.norm.0,
                                ...
                                box.obs.farm,
                                ncol = 2,
                                top = tableGrob(t("lmer(inverts_BC_all ~ day*soil_ES*treatment*Region + (1|Farm)"),
    ...
    final.plot3 <- grid.arrange(box.trt,
                                box.region,
                                box.trt.region,
                                box.farm,
                                reqq.plot.farm,
    ...
    final.plot5 <- grid.arrange(g6,g8,g5,g7,
                                ncol = 2,
                                top = tableGrob(t("lmer(inverts_BC_all ~ day*soil_ES*treatment*Region + (1|Farm)"),
                                                theme = ttheme_minimal(padding = unit(c(0,8),'mm'),
                                                                       base_colour = "black",
    ...
    obs_var_trt_region[m] <- OBS_var_trt_region$`Pr(>F)`[1]
    ...
    resid_var_trt_region[m] <- RESID_var_trt_region$`Pr(>F)`[1]
...
mod_summary <- data.frame(timepoint.hr, width.hr, 
                          obs_norm, obs_var_trt, obs_var_region, obs_var_trt_region, obs_var_farm, 
                          resid_norm, resid_var_trt, resid_var_region, resid_var_trt_region, resid_var_farm, 
                          mod_AIC, mod_Rsq_marginal, mod_Rsq_conditional)

#================================================
# Export summary and output of the models

write.table(mod_summary, paste0(mydir.data, "/LMER Model summary - inverts BC vs soil moisture trt region.csv"), 
            sep = ",", row.names = F)

write.table(aov_mod_final, paste0(mydir.data, "/LMER Anova output - inverts BC vs soil moisture trt region.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out_final, paste0(mydir.data, "/LMER Model output fixef - inverts BC vs soil moisture trt region.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.1, paste0(mydir.data, "/LMER Model output random Region - inverts BC vs soil moisture trt region.csv"), 
            sep = ",", row.names = F)
...
gp1 <- ggplot(mod_summary, aes(x = width.hr, y = mod_AIC)) +
  ...
  labs(x = expression(paste(bold("Window width (h)"))),
       ...
       subtitle = "Fixed: day * soil moisture effect size * treatment * Region\nRandom: Farm\nDaytime: 12:00 h") +
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - AIC vs Window width and mar R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - AIC vs Window width and con R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - AIC vs Window width R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Marginal R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Conditional R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Shapiro test.png"),
       p1, width = 20, height = 16, units = "cm")

# leveneTest ~ treatment

p2 <- ggplot(mod_summary, aes(x = width.hr, y = resid_var_trt)) +
  geom_point(aes(color = mod_Rsq_conditional, fill = mod_Rsq_conditional, size = mod_AIC), 
             shape = 21, alpha = 0.5) +
  scale_color_viridis_c(name = con.Rsq) +
  scale_fill_viridis_c(name = con.Rsq) + 
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Levene Test Trt.png"),
       p2, width = 20, height = 16, units = "cm")

# leveneTest ~ Region

p3 <- ggplot(mod_summary, aes(x = width.hr, y = resid_var_region)) +
  geom_point(aes(color = mod_Rsq_conditional, fill = mod_Rsq_conditional, size = mod_AIC), 
             shape = 21, alpha = 0.5) +
  scale_color_viridis_c(name = con.Rsq) +
  scale_fill_viridis_c(name = con.Rsq) + 
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Levene Test Region.png"),
       p3, width = 20, height = 16, units = "cm")

# leveneTest ~ treatment * Region

p4 <- ggplot(mod_summary, aes(x = width.hr, y = resid_var_trt_region)) +
  geom_point(aes(color = mod_Rsq_conditional, fill = mod_Rsq_conditional, size = mod_AIC), 
             shape = 21, alpha = 0.5) +
  scale_color_viridis_c(name = con.Rsq) +
  scale_fill_viridis_c(name = con.Rsq) + 
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Levene Test Trt Region.png"),
       p4, width = 20, height = 16, units = "cm")

# leveneTest ~ Farm

p5 <- ggplot(mod_summary, aes(x = width.hr, y = resid_var_farm)) +
  geom_point(aes(color = mod_Rsq_conditional, fill = mod_Rsq_conditional, size = mod_AIC), 
             shape = 21, alpha = 0.5) +
  scale_color_viridis_c(name = con.Rsq) +
  scale_fill_viridis_c(name = con.Rsq) + 
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Levene Test Farm.png"),
       p5, width = 20, height = 16, units = "cm")
...
p6 <- ggplot(mod_output, aes(x = Duration, y = p.value)) +
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - P values vs Window width.png"),
       p6, width = 30, height = 24, units = "cm")
...
p7 <- ggplot(mod_sig, aes(x = Term, y = freq)) +
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture, Trt and Region - Significance of the terms.png"),
       p7, width = 18, height = 15, units = "cm")
