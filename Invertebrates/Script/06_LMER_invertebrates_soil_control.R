...
## moisture in each plot C1-C3 minus the average soil moisture in control plots) ##
...
##                                                                               ##
## In this script, the model is fitted only using the data from control plots    ##
##                                                                               ##
...
# dir.create(paste0(data.dir,"LMER model invertebrates soil control"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil control"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil control/Model validation"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil control/Model output"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil control/Residuals"))

mydir.data <- paste0(data.dir,"LMER model invertebrates soil control")
mydir <- paste0(figures.dir,"LMER model invertebrates soil control")
...
for (daytime.hr in timepoint) {
  ...
  for (duration.hr in width) {
    ...
    # Create a subset containing only the control plots
    
    df_joined <- df_joined %>% filter(treatment == "Control") %>% droplevels()
    ...
    if (daytime.hr < 10) {
      title <- paste0("LMER Model ",duration.hr, " h period, starting at 0",daytime.hr,":00, Control plots")
    ...
    if (daytime.hr >= 10) {
      title <- paste0("LMER Model ",duration.hr, " h period, starting at ",daytime.hr,":00, Control plots")
    ...
    g1 <- ggplot(df_mod, 
      ...
      labs(x = "Days since the end of the drought",
           title = "Fitted data - Control plots",
...
write.table(mod_summary, paste0(mydir.data, "/LMER Model summary - inverts BC vs soil moisture Control.csv"), 
            sep = ",", row.names = F)

write.table(aov_mod_final, paste0(mydir.data, "/LMER Anova output - inverts BC vs soil moisture Control.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out_final, paste0(mydir.data, "/LMER Model output fixef - inverts BC vs soil moisture Control.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.1, paste0(mydir.data, "/LMER Model output random Region - inverts BC vs soil moisture Control.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.2, paste0(mydir.data, "/LMER Model output random Farm Region - inverts BC vs soil moisture Control.csv"), 
            sep = ",", row.names = F)
...
gp1 <- ggplot(mod_summary, aes(x = width.hr, y = mod_AIC)) +
  ...
  labs(x = expression(paste(bold("Window width (h)"))),
       ...
       title = "LMER - Invertebrates Bray-Curtis Dissimilarity - Control plots",
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - AIC vs Window width and mar R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - AIC vs Window width and con R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - AIC vs Window width R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Marginal R2 vs AIC.png"),
...
gp4 <- ggplot(mod_summary, aes(x = mod_AIC, y = mod_Rsq_conditional,                                
  ...
  labs(x = expression(paste(bold("AIC"))),
       y = con.Rsq,
       title = "LMER - Invertebrates Bray-Curtis Dissimilarity - Control",
       subtitle = "Fixed: day * soil moisture effect size\nRandom: Region/Farm\nDaytime: 12:00 h")

ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Conditional R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Shapiro test.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Levene Test Region.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Levene Test Farm.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Levene Test Region Farm.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - P values vs Window width.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Control - Significance of the terms.png"),
