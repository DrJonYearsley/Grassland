...
# dir.create(paste0(data.dir,"LMER model invertebrates trt region"))
# dir.create(paste0(figures.dir,"LMER model invertebrates trt region"))
# dir.create(paste0(figures.dir,"LMER model invertebrates trt region/Model validation"))
# dir.create(paste0(figures.dir,"LMER model invertebrates trt region/Model output"))
# dir.create(paste0(figures.dir,"LMER model invertebrates trt region/Residuals"))

mydir.data <- paste0(data.dir,"LMER model invertebrates trt region")
mydir <- paste0(figures.dir,"LMER model invertebrates trt region")
...
for (daytime.hr in timepoint) {
  ...
  for (duration.hr in width) {
    ...
    lmer_full_mod <- lmer(inverts_BC_all ~ day*treatment*Region + (1|Farm),
                          data = df_joined)
    ...
    final.plot0 <- grid.arrange(plot.norm.0,
                                ...
                                box.obs.farm,
                                ncol = 2,
                                top = tableGrob(t("lmer(inverts_BC_all ~ day*treatment*Region + (1|Farm)"),
...
write.table(mod_summary, paste0(mydir.data, "/LMER Model summary - inverts BC vs trt region.csv"), 
            sep = ",", row.names = F)

write.table(aov_mod_final, paste0(mydir.data, "/LMER Anova output - inverts BC vs trt region.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out_final, paste0(mydir.data, "/LMER Model output fixef - inverts BC vs trt region.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.1, paste0(mydir.data, "/LMER Model output random Region - inverts BC vs trt region.csv"), 
            sep = ",", row.names = F)
...
gp1 <- ggplot(mod_summary, aes(x = width.hr, y = mod_AIC)) +
  ...
  labs(x = expression(paste(bold("Window width (h)"))),
       ...
       subtitle = "Fixed: day * treatment * Region\nRandom: Farm\nDaytime: 12:00 h") +
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - AIC vs Window width and mar R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - AIC vs Window width and con R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Marginal R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Conditional R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Shapiro test.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Levene Test Trt.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Levene Test Region.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Levene Test Trt Region.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Levene Test Farm.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - P values vs Window width.png"),
       p6, width = 24, height = 24, units = "cm")

# p-values vs. window widthm y range adjusted

p7 <- ggplot(mod_output, aes(x = Duration, y = p.value)) +
  geom_point(aes(color = mod_Rsq_marginal, fill = mod_Rsq_marginal), 
             shape = 21, alpha = 0.3, size = 3) +
  facet_wrap(~ Term, ncol = 3) +
  scale_color_viridis_c(name = mar.Rsq) +
  ...
  scale_y_continuous(limits = c(0,0.25), 
                     ...
                     labels = c("0",expression(paste(bold("0.05"))),"0.1","0.15","0.2","0.25")) +
  geom_vline(xintercept = -0.5, color = "grey80", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = -15.5, color = "grey80", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = -38.5, color = "grey80", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = -79.5, color = "grey80", linetype = "dashed", size = 0.5)

ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - P values vs Window width V2.png"),
       p7, width = 24, height = 24, units = "cm")
...
p8 <- ggplot(mod_sig, aes(x = Term, y = freq)) +
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Trt and Region - Significance of the terms.png"),
       p8, width = 18, height = 15, units = "cm")
