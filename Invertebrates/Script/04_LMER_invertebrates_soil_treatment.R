...
# dir.create(paste0(data.dir,"LMER model invertebrates soil trt"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt/Model validation"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt/Model output"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt/Residuals"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt/Soil moisture effect size boxplot"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt/Soil moisture effect size V1"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil trt/Soil moisture effect size V2"))

mydir.data <- paste0(data.dir,"LMER model invertebrates soil trt")
mydir <- paste0(figures.dir,"LMER model invertebrates soil trt")
...
for (daytime.hr in timepoint) {
  ...
  for (duration.hr in width) {
    ...
    final.plot0 <- grid.arrange(plot.norm.0,
                                ...
                                box.obs.region.farm,
                                ncol = 2,
                                top = tableGrob(t("lmer(inverts_BC_all ~ day*soil_ES*treatment + (1|Region/Farm)"),
...
write.table(mod_summary, paste0(mydir.data, "/LMER Model summary - inverts BC vs soil moisture trt.csv"), 
            sep = ",", row.names = F)

write.table(aov_mod_final, paste0(mydir.data, "/LMER Anova output - inverts BC vs soil moisture trt.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out_final, paste0(mydir.data, "/LMER Model output fixef - inverts BC vs soil moisture trt.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.1, paste0(mydir.data, "/LMER Model output random Region - inverts BC vs soil moisture trt.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.2, paste0(mydir.data, "/LMER Model output random Farm Region - inverts BC vs soil moisture trt.csv"), 
            sep = ",", row.names = F)
...
gp1 <- ggplot(mod_summary, aes(x = width.hr, y = mod_AIC)) +
  ...
  labs(x = expression(paste(bold("Window width (h)"))),
       ...
       subtitle = "Fixed: day * soil moisture effect size * treatment\nRandom: Region/Farm\nDaytime: 12:00 h") +
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - AIC vs Window width and mar R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - AIC vs Window width and con R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - AIC vs Window width R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Marginal R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Conditional R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Shapiro test.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Levene Test Region.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Levene Test Farm.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Levene Test Region Farm.png"),
...
p5 <- ggplot(mod_output, aes(x = Duration, y = p.value)) +
  geom_point(aes(color = mod_Rsq_conditional, fill = mod_Rsq_conditional), 
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - P values vs Window width.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture and Trt - Significance of the terms.png"),
