...
## moisture in each plot D1-D3 minus the average soil moisture in control plots) ##
...
## invertebrate community composition in drought plots at each site and each     ##
## sampling day (9, 32, 64), i.e. reference community)                           ##
##                                                                               ##
## In this script, the model is fitted only using the data from drought plots    ##
##                                                                               ##
...
# dir.create(paste0(data.dir,"LMER model invertebrates soil drought"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil drought"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil drought/Model validation"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil drought/Model output"))
# dir.create(paste0(figures.dir,"LMER model invertebrates soil drought/Residuals"))

mydir.data <- paste0(data.dir,"LMER model invertebrates soil drought")
mydir <- paste0(figures.dir,"LMER model invertebrates soil drought")
...
for (daytime.hr in timepoint) {
  ...
  for (duration.hr in width) {
    ...
    # Create a subset containing only the drought plots
    
    df_joined <- df_joined %>% filter(treatment == "Drought") %>% droplevels()
    ...
    # Fit a model of the ratio of the plot to the mean of the drought plots as 
    ...
    lmer_full_mod <- lmer(inverts_BC_all ~ day*soil_ES + (1|Region/Farm),
                          data = df_joined)
    ...
    if (daytime.hr < 10) {
      title <- paste0("LMER Model ",duration.hr, " h period, starting at 0",daytime.hr,":00, Drought plots")
    ...
    if (daytime.hr >= 10) {
      title <- paste0("LMER Model ",duration.hr, " h period, starting at ",daytime.hr,":00, Drought plots")
    ...
    final.plot0 <- grid.arrange(plot.norm.0,
                                ...
                                box.obs.region.farm,
                                ncol = 2,
                                top = tableGrob(t("lmer(inverts_BC_all ~ day*soil_ES + (1|Region/Farm)"),
    ...
    g1 <- ggplot(df_mod, 
      ...
      labs(x = "Days since the end of the drought",
           title = "Fitted data - Drought plots",
...
write.table(mod_summary, paste0(mydir.data, "/LMER Model summary - inverts BC vs soil moisture Drought.csv"), 
            sep = ",", row.names = F)

write.table(aov_mod_final, paste0(mydir.data, "/LMER Anova output - inverts BC vs soil moisture Drought.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out_final, paste0(mydir.data, "/LMER Model output fixef - inverts BC vs soil moisture Drought.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.1, paste0(mydir.data, "/LMER Model output random Region - inverts BC vs soil moisture Drought.csv"), 
            sep = ",", row.names = F)

write.table(random_out_final.2, paste0(mydir.data, "/LMER Model output random Farm Region - inverts BC vs soil moisture Drought.csv"), 
            sep = ",", row.names = F)
...
gp1 <- ggplot(mod_summary, aes(x = width.hr, y = mod_AIC)) +
  ...
  labs(x = expression(paste(bold("Window width (h)"))),
       ...
       title = "LMER - Invertebrates Bray-Curtis Dissimilarity - Drought plots",
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - AIC vs Window width and mar R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - AIC vs Window width and con R2.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - AIC vs Window width R2.png"),
       gp1 | gp2, width = 32, height = 18, units = "cm")
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Marginal R2 vs AIC.png"),
...
gp4 <- ggplot(mod_summary, aes(x = mod_AIC, y = mod_Rsq_conditional,                                
  ...
  labs(x = expression(paste(bold("AIC"))),
       y = con.Rsq,
       title = "LMER - Invertebrates Bray-Curtis Dissimilarity - Drought",
       subtitle = "Fixed: day * soil moisture effect size\nRandom: Region/Farm\nDaytime: 12:00 h")

ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Conditional R2 vs AIC.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Shapiro test.png"),
...
p2 <- ggplot(mod_summary, aes(x = width.hr, y = resid_var_region)) +
  ...
  labs(x = "Window width (h)", y = ylab, 
       title = "LMER - Invertebrates Bray-Curtis Dissimilarity - Drought plots\nResiduals ~ Region",
       subtitle = "Fixed: day * soil moisture effect siz\nRandom: Region/Farm\nDaytime: 12:00 h") +
  geom_vline(xintercept = -0.5, color = "grey80", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = -15.5, color = "grey80", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = -38.5, color = "grey80", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = -79.5, color = "grey80", linetype = "dashed", size = 0.5)

ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Levene Test Region.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Levene Test Farm.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Levene Test Region Farm.png"),
...
mod_output$Term <- factor(mod_output$Term, levels = c("day","soil_ES","day:soil_ES"))
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - P values vs Window width.png"),
...
ggsave(paste0(mydir, "/LMER Model validation Inverts vs Soil Moisture Drought - Significance of the terms.png"),
