...
dir.create(paste0(figures.dir,"LMER model inverts BC day trt 2"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model inverts BC day trt 2")
...
lmer_full_mod <- lmer(inverts_BC_all ~ day*treatment + (1|Region/Farm),
...
title.model <- "lmer(inverts_BC_all~ day*treatment + (1|Region/Farm))"
...
AIC(lmer_full_mod)                                  ## -298.3722
model_performance(lmer_full_mod)$R2_marginal        ## 0.2017385
model_performance(lmer_full_mod)$R2_conditional     ## 0.2246817

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - ",resp.variable," vs Day and Trt 2.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - ",resp.variable," vs Day and Trt 2.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm Region - ",resp.variable," vs Day and Trt 2.csv"), 
            sep = ",", row.names = F)

write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Region - ",resp.variable," vs Day and Trt 2.csv"), 
            sep = ",", row.names = F)
