...
## between middrought and a given sampling day. This would then correspond to    ##
## the same time window as the plant biomass (non repeated measures). This means,##
...
## for day 8 is calculated based on the average soil moisture between middrought ##
...
for (i in 1:length(sites)) {
  ...
  day8.interval <- day8 - hours(0:(24*(8+mid.day0.duration)-1))
  day16.interval <- day16 - hours(0:(24*(16+mid.day0.duration)-1))
  day32.interval <- day32 - hours(0:(24*(32+mid.day0.duration)-1))
  day64.interval <- day64 - hours(0:(24*(64+mid.day0.duration)-1))
  
  ## Extract soil moisture for each interval
  
  moisture.day0 <-  mysite %>% 
    filter(date.time %in% day0.interval)
  moisture.day0$day <- 0
  
  moisture.day8 <-  mysite %>% 
    filter(date.time %in% day8.interval)
  moisture.day8$day <- 8
  
  moisture.day16 <-  mysite %>% 
    filter(date.time %in% day16.interval)
  moisture.day16$day <- 16
  
  moisture.day32 <-  mysite %>% 
    filter(date.time %in% day32.interval)
  moisture.day32$day <- 32
  
  moisture.day64 <-  mysite %>% 
    filter(date.time %in% day64.interval)
  moisture.day64$day <- 64
  
  ## Combine all subsets together
  
  moisture.site <- rbind(moisture.day0,
                         moisture.day8,
                         moisture.day16,
                         moisture.day32,
                         moisture.day64)
  ...
  rm(day0,day8,day16,day32,day64,
     ...
     moisture.day0,moisture.day8,moisture.day16,moisture.day32,moisture.day64,
     mid.drought,mid.day0.duration,mysite,moisture.site)
}
...
resilience_soil_non_rm <- moisture.window

# Rename C to Control and D to drought

resilience_soil_non_rm$treatment <- gsub("C", "Control", 
                                         as.character(resilience_soil_non_rm$treatment))

resilience_soil_non_rm$treatment <- gsub("D", "Drought", 
                                         as.character(resilience_soil_non_rm$treatment))

# Change Region, Farm and treatment to factor

resilience_soil_non_rm$Region <- as.factor(resilience_soil_non_rm$Region)
resilience_soil_non_rm$Farm <- as.factor(resilience_soil_non_rm$Farm)
resilience_soil_non_rm$treatment <- as.factor(resilience_soil_non_rm$treatment)
...
resilience_soil_non_rm <- left_join(resilience_soil_non_rm,
                                    rain.acc[,c("Region","Farm","Acc.Rain.D")],
                                    by = c("Region","Farm"))

# Rename Acc.Rain.D to rain_out

names(resilience_soil_non_rm)[names(resilience_soil_non_rm) == "Acc.Rain.D"] <- "acc_rain"

# Replace accumulated rainfall with zero for all drought plots

resilience_soil_non_rm$acc_rain[resilience_soil_non_rm$treatment == "Drought"] <- 0
...
write.table(resilience_soil_non_rm, paste0(data.dir,"Resilience_Soil_with_Rainfall_non_rm.csv"), sep = ",", row.names = F)
save(resilience_soil_non_rm, file = paste0(data.dir,"Resilience_Soil_with_Rainfall_non_rm.RData"))
...
g1 <- ggplot(resilience_soil_non_rm,aes(x = day, y = plot_meanC_ratio)) + 
  ...
  scale_y_continuous(limits = c(-0.1,2.1), exp = c(0,0)) +
  ...
  labs(x = "Days since the end of the simulated drought",
       ...
       subtitle = "Starting at mid-drought (non rm)") 

g2 <- ggplot(resilience_soil_non_rm,aes(x = day, y = plot_meanC_ratio)) + 
...
ggsave("Figures/Soil moisture ratio - Middrought (non rm) V1.png", g1, width = 10, height = 8)
ggsave("Figures/Soil moisture ratio - Middrought (non rm) V2.png", g2, width = 10, height = 8)
