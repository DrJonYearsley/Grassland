...
dir.create(paste0(figures.dir,"LMER model plant biomass day trt"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model plant biomass day trt")
...
# Check normality of the data (plant biomass ratio) ----

check.norm <- ggHistNorm(resilience_plant, resilience_plant$plot_meanC_ratio, "Plant biomass ratio", 0.05)
plot.norm <- check.norm[1][[1]]

plot.qq <- ggQQplot(resilience_plant, resilience_plant$plot_meanC_ratio, "Plant biomass ratio")

# Treatment Control

C_resilience <- resilience_plant %>% filter(treatment == "Control")

check.norm.C <- ggHistNorm(C_resilience, C_resilience$plot_meanC_ratio, "Plant biomass ratio - Control", 0.05)
plot.norm.C <- check.norm.C[1][[1]]

plot.qq.C <- ggQQplot(C_resilience, C_resilience$plot_meanC_ratio, "Plant biomass ratio - Control")

# Treatment Drought

D_resilience <- resilience_plant %>% filter(treatment == "Drought")

check.norm.D <- ggHistNorm(D_resilience, D_resilience$plot_meanC_ratio, "Plant biomass ratio - Drought", 0.05)
plot.norm.D <- check.norm.D[1][[1]]

plot.qq.D <- ggQQplot(D_resilience, D_resilience$plot_meanC_ratio, "Plant biomass ratio - Drought")
...
OBS_var_day <- leveneTest(plot_meanC_ratio ~ as.factor(day), data = resilience_plant)
...
box.obs.day <- ggplot(resilience_plant, aes(x = as.factor(day), 
                                            y = plot_meanC_ratio, 
                                            fill = as.factor(day))) +
...
OBS_var_trt <- leveneTest(plot_meanC_ratio ~ treatment, data = resilience_plant)
...
box.obs.trt <- ggplot(resilience_plant, aes(x = treatment, y = plot_meanC_ratio, fill = treatment)) +
  geom_boxplot(outlier.shape = 21) +
  theme_minimal() +
  theme(legend.position = "none",
        axis.title = element_text(size = 12, face = "bold")) +
...
OBS_var_region <- leveneTest(plot_meanC_ratio ~ Region, data = resilience_plant)
...
box.obs.region <- ggplot(resilience_plant, aes(x = Region, y = plot_meanC_ratio, fill = Region)) +
...
OBS_var_farm <- leveneTest(plot_meanC_ratio ~ Farm, data = resilience_plant)
...
box.obs.farm <- ggplot(resilience_plant, aes(x = Farm, y = plot_meanC_ratio, fill = Farm)) +
  geom_boxplot(outlier.shape = 21, alpha = 0.7) +
...
OBS_var_region_farm <- leveneTest(plot_meanC_ratio ~ Region*Farm, data = resilience_plant)
...
box.obs.region.farm <- ggplot(resilience_plant, aes(x = Region, y = plot_meanC_ratio, fill = Farm)) +
  geom_boxplot(outlier.shape = 21, alpha = 0.7) +
  scale_fill_brewer(palette = "GnBu") +
  theme_minimal() +
  theme(legend.position = "none",
...
figure.title1 <- paste0(mydir,"/Model assumptions - Plant biomass ratio vs Day and Treatment V1.png")
...
figure.title2 <- paste0(mydir,"/Model assumptions - Plant biomass ratio vs Day and Treatment V2.png")
...
AIC(lmer_full_mod)                                  ## -3.854633
model_performance(lmer_full_mod)$R2_marginal        ## 0.3113547, Random effect variances not available. Returned R2 does not account for random effects.  
model_performance(lmer_full_mod)$R2_conditional     ## NA

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - Plant biomass ratio vs Day and Trt.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - Plant biomass ratio vs Day and Trt.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm Region - Plant biomass ratio vs Day and Trt.csv"), 
            sep = ",", row.names = F)

write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Region - Plant biomass ratio vs Day and Trt.csv"), 
            sep = ",", row.names = F)

write.table(random_out.3, paste0(mydir.data, "/LMER Model output random Day - Plant biomass ratio vs Day and Trt.csv"), 
            sep = ",", row.names = F)