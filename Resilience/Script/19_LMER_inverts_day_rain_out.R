...
## simulated drought (covariate) and with random effects day (see above)         ##
...
dir.create(paste0(figures.dir,"LMER model inverts BC day rain out"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model inverts BC day rain out")
...
lmer_full_mod <- lmer(inverts_BC_all ~ day*rain_out + (1|day) + (1|Region/Farm),
...
title.model <- "lmer(inverts_BC_all~ day*rain_out + (1|day) + (1|Region/Farm))"
...
AIC(lmer_full_mod)                                  ## -308.9029
model_performance(lmer_full_mod)$R2_marginal        ## 0.165733
model_performance(lmer_full_mod)$R2_conditional     ## 0.3769807

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - ",resp.variable," vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - ",resp.variable," vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm Region - ",resp.variable," vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Region - ",resp.variable," vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(random_out.3, paste0(mydir.data, "/LMER Model output random Day - ",resp.variable," vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)
