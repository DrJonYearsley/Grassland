...
dir.create(paste0(figures.dir,"LMER model inverts BC day trt region 2"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model inverts BC day trt region 2")
...
title.model <- "lmer(inverts_BC_all ~ day*treatment*Region + (1|Farm))"
...
AIC(lmer_full_mod)                                  ## -196.0569
model_performance(lmer_full_mod)$R2_marginal        ## 0.237433
model_performance(lmer_full_mod)$R2_conditional     ## 0.252223

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - ",resp.variable," vs Day, Trt and Region 2.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - ",resp.variable," vs Day, Trt and Region 2.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm - ",resp.variable," vs Day, Trt and Region 2.csv"), 
            sep = ",", row.names = F)
