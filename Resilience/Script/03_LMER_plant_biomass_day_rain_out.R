...
dir.create(paste0(figures.dir,"LMER model plant biomass day rain out"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model plant biomass day rain out")
...
AIC(lmer_full_mod)                                  ## 19.69411
model_performance(lmer_full_mod)$R2_marginal        ## 0.3015246
model_performance(lmer_full_mod)$R2_conditional     ## 0.3366555

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - Plant biomass ratio vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - Plant biomass ratio vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm Region - Plant biomass ratio vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Region - Plant biomass ratio vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)

write.table(random_out.3, paste0(mydir.data, "/LMER Model output random Day - Plant biomass ratio vs Day and Rain Out.csv"), 
            sep = ",", row.names = F)
