...
## first modified: 08 Mar 2020                                                   ##
## last modified: 08 Mar 2020                                                    ##
###################################################################################
...
dir.create(paste0(figures.dir,"LMER model inverts BC day trt region"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model inverts BC day trt region")
...
title.model <- "lmer(inverts_BC_all ~ day*treatment*Region + (1|day) + (1|Farm))"

# Check for normal distribution within each region and treatment

region <- unique(resilience_inverts$Region)
trt <- unique(resilience_inverts$treatment)

for(i in 1:length(region)){
  
  # Extract relevant data for the respective region and treatment
  
  resilience_C <- resilience_inverts %>% filter(Region == region[i] & treatment == "Control") %>% droplevels()
  resilience_D <- resilience_inverts %>% filter(Region == region[i] & treatment == "Drought") %>% droplevels()
  
  region_resilience <- resilience_inverts %>% filter(Region == region[i]) %>% droplevels()
  
  # Use function ggHistNorm
  
  check.norm.C <- ggHistNorm(resilience_C, resilience_C$inverts_BC_all, 
                             paste0(resp.variable," - ",region[i],", Control plots"), 0.05)
  
  plot.norm.C <- check.norm.C[1][[1]]
  
  check.norm.D <- ggHistNorm(resilience_D, resilience_D$inverts_BC_all, 
  ...
  plot.qq.C <- ggQQplot(resilience_C, resilience_C$inverts_BC_all, 
                        paste(resp.variable," - ",region[i],", Control plots"))
  
  plot.qq.D <- ggQQplot(resilience_D, resilience_D$inverts_BC_all, 
                        paste(resp.variable," - ",region[i],", Drought plots"))
  
  
  # Combine all four plots in one figure
  
  figure.title.region <- paste0(mydir,"/Model assumptions - ",resp.variable," vs Day, Trt and Region ",region[i],".png")
...
lmer_full_mod <- lmer(inverts_BC_all ~ day*treatment*Region + (1|day) + (1|Farm),
...
AIC(lmer_full_mod)                                  ## -231.1451
model_performance(lmer_full_mod)$R2_marginal        ## 0.2024568
model_performance(lmer_full_mod)$R2_conditional     ## 0.3954951

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - ",resp.variable," vs Day, Trt and Region.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - ",resp.variable," vs Day, Trt and Region.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm - ",resp.variable," vs Day, Trt and Region.csv"), 
            sep = ",", row.names = F)

write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Day - ",resp.variable," vs Day, Trt and Region.csv"), 
            sep = ",", row.names = F)
