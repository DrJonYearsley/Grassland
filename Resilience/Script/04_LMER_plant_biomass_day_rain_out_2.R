...
dir.create(paste0(figures.dir,"LMER model plant biomass day rain out 2"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model plant biomass day rain out 2")
...
# Combine rainfall with plant biomass data

resilience_plant <- left_join(resilience_plant,
                              rain.acc[,c("Region","Farm","Acc.Rain.D")],
                              by = c("Region","Farm"))

# Rename Acc.Rain.D to rain_out

names(resilience_plant)[names(resilience_plant) == "Acc.Rain.D"] <- "rain_out"

# Replace accumulated rainfall with zero for all drought plots

resilience_plant$rain_out[resilience_plant$treatment == "Drought"] <- 0
...
lmer_full_mod <- lmer(plot_meanC_ratio ~ day*rain_out + (1|Region/Farm),
...
title.model <- "lmer(plot_meanC_ratio ~ day*rain_out + (1|Region/Farm))"
...
midpoint <- min(resilience_plant$rain_out[resilience_plant$rain_out > 0], na.rm = T)
...
AIC(lmer_full_mod)                                  ## 17.69411
model_performance(lmer_full_mod)$R2_marginal        ## 0.3125038, Random effect variances not available. Returned R2 does not account for random effects.  
model_performance(lmer_full_mod)$R2_conditional     ## NA 

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - Plant biomass ratio vs Day and Rain Out 2.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - Plant biomass ratio vs Day and Rain Out 2.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm Region - Plant biomass ratio vs Day and Rain Out 2.csv"), 
            sep = ",", row.names = F)

write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Region - Plant biomass ratio vs Day and Rain Out 2.csv"), 
            sep = ",", row.names = F)