---
title: "Soil moisture ratio non rm - Model comparison for treatment"
author: "Dr. Maja Ilić"
date: "4/22/2020"
...
```{r}
rand2_nlminb <- lmer(plot_meanC_ratio ~ day * treatment + (1|day) + (treatment|Region),
                     data = resilience_soil_non_rm,
                     REML = T,
                     control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,
                                           optCtrl = list(method = "nlminb", 
...
```{r}
rand3_nlminb <- lmer(plot_meanC_ratio ~ day * treatment + (1|Farm) + (treatment|Region),
                     data = resilience_soil_non_rm,
                     REML = T,
                     control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,
                                           optCtrl = list(method = "nlminb", 
...
```{r fig.width = 8, fig.height = 7}
out4 <- data.frame(resilience_soil_non_rm, "Fitted" = fitted(rand4))  
...
## Refitting the model with the optimizer L-BFGS-B:  

```{r}
rand10_L_BFGS_B <- lmer(plot_meanC_ratio ~ day * treatment + (1|Region/Farm) + (treatment - 1|Region),
                        data = resilience_soil_non_rm,
                        REML = T,
                        control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,
...
```{r fig.width = 7, fig.height = 8, warning = F, message = F}
check_model(rand10_L_BFGS_B)
```

## SD and correlation of the random effects:  

```{r}
summary(rand10_L_BFGS_B)$varcor
```

## Variance of the random effects:  

```{r warning = F}
pander(summary(rand10_L_BFGS_B)$varcor, style = 'rmarkdown')
```

## Estimates for the random effects:  

```{r warning = F}
pander(ranef(rand10_L_BFGS_B), style = 'rmarkdown')
```

## Significance of the random effects:  

```{r message = F}
pander(rand(rand10_L_BFGS_B)[c(2,3,5,6)], style = 'rmarkdown')
```

## Significance of the fixed effects:  

```{r}
pander(anova(rand10_L_BFGS_B)[-3], style = 'rmarkdown')
```

## Visualize the model output using fitted values:  

```{r fig.width = 8, fig.height = 7}
out10 <- data.frame(resilience_soil_non_rm, "Fitted" = fitted(rand10_L_BFGS_B))  
...
ggplot(out10, 
  ...
  labs(x = "Days since the end of the simulated drought",
       ...
       subtitle = "lm(fitted data ~ day), optimizer: L-BFGS-B")
