...
## and with random effect Farm (1-5)                                             ##
...
dir.create(paste0(figures.dir,"LMER model plant biomass day trt region 2"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model plant biomass day trt region 2")
...
# Random factors included: Farm (1-5)

lmer_full_mod <- lmer(plot_meanC_ratio ~ day*treatment*Region + (1|Farm),
...
title.model <- "lmer(plot_meanC_ratio ~ day*treatment*Region + (1|Farm))"
...
AIC(lmer_full_mod)                                  ## 112.6075
model_performance(lmer_full_mod)$R2_marginal        ## 0.3085886
model_performance(lmer_full_mod)$R2_conditional     ## 0.318263

write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - Plant biomass ratio vs Day, Trt and Region 2.csv"), 
            sep = ",", row.names = F)

write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - Plant biomass ratio vs Day, Trt and Region 2.csv"), 
            sep = ",", row.names = F)

write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm - Plant biomass ratio vs Day, Trt and Region 2.csv"), 
            sep = ",", row.names = F)