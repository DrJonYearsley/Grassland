...
## simulated drought (covariate), and with random effects Farm (1-5) nested in   ##
...
dir.create(paste0(figures.dir,"LMER model soil moisture day rain out 2"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model soil moisture day rain out 2")
...
AIC(lmer_full_mod)                                  ## -92.14057
model_performance(lmer_full_mod)$R2_marginal        ## 0.205931
model_performance(lmer_full_mod)$R2_conditional     ## 0.36555

# write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - Soil moisture ratio vs Day and Rain Out 2.csv"), 
#             sep = ",", row.names = F)
# 
# write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - Soil moisture ratio vs Day and Rain Out 2.csv"), 
#             sep = ",", row.names = F)
# 
# write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm Region - Soil moisture ratio vs Day and Rain Out 2.csv"), 
#             sep = ",", row.names = F)
# 
# write.table(random_out.2, paste0(mydir.data, "/LMER Model output random Region - Soil moisture ratio vs Day and Rain Out 2.csv"), 
#             sep = ",", row.names = F)
