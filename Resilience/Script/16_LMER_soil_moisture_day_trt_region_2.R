...
dir.create(paste0(figures.dir,"LMER model soil moisture day trt region 2"))

mydir.data <- data.dir
mydir <- paste0(figures.dir,"LMER model soil moisture day trt region 2")
...
AIC(lmer_full_mod)                                  ## -20.85255
model_performance(lmer_full_mod)$R2_marginal        ## 0.327862
model_performance(lmer_full_mod)$R2_conditional     ## 0.3405135

# write.table(aov_mod, paste0(mydir.data, "/LMER Anova output - Soil moisture ratio vs Day, Trt and Region 2.csv"), 
#             sep = ",", row.names = F)
# 
# write.table(fixef_out, paste0(mydir.data, "/LMER Model output fixef - Soil moisture ratio vs Day, Trt and Region 2.csv"), 
#             sep = ",", row.names = F)
# 
# write.table(random_out.1, paste0(mydir.data, "/LMER Model output random Farm - Soil moisture ratio vs Day, Trt and Region 2.csv"), 
#             sep = ",", row.names = F)
